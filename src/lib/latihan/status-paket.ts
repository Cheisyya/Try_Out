 ...
 * (Postgres pada Vercel, berkas lokal pada pengembangan) — bukan layanan KV
 * terpisah yang memerlukan konfigurasi environment tambahan.
 *
...
export async function bacaOverlay(kunci: string): Promise<OverlayStatus> {
  ...
  const paket =
    tersimpan?.paket && typeof tersimpan.paket === "object"
      ...
      : {};
  const sesi =
    tersimpan?.sesi && typeof tersimpan.sesi === "object" ? tersimpan.sesi : {};
  return { paket, sesi };
}
...
export function aktifPaketDariOverlay(
  ...
  if (Object.hasOwn(overlay.paket, paketId)) return overlay.paket[paketId];
...
export function aktifSesiDariOverlay(
  ...
  if (paket && Object.hasOwn(paket, sesiId)) return paket[sesiId];
  return aktifAsli !== false;
}
...
>(daftar: T[], overlay: OverlayStatus): T[] {
  return daftar.map((paket) => {
    if (!Object.hasOwn(overlay.paket, paket.id)) return paket;
...
>(paket: P, overlay: OverlayStatus): P {
  ...
  return {
    ...
    sesi: paket.sesi.map((sesi) =>
      Object.hasOwn(sesiOverlay, sesi.id)
